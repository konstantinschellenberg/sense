...
class Dobson85(EpsModel):
    ...
    def _debye(self):
        ...
        tau_w = (
                            1.1109 * 10 ** -10 - 3.824 * 10 ** -12 * self.t + 6.938 * 10 ** -14 * self.t ** 2 - 5.096 * 10 ** -16 * self.t ** 3) / 2. / np.pi
        ...
        if self.single_debye:
            ...
            e2 = 2 * np.pi * f * tau_w * (ew_0 - ew_inf) / (1 + (2 * np.pi * f * tau_w) ** 2) + (
                        2.65 - self.bulk) / 2.65 / self.mv * self.sigma / (2 * np.pi * 8.854 * 10 ** -12 * f)
        return e1 + 1.j * e2
    ...
    def _calc_eps(self):
        ...
        e1 = (1. + 0.66 * self.bulk + self.mv ** self.beta1 * np.real(self.ew) ** self.alpha - self.mv) ** (
                    1. / self.alpha)
