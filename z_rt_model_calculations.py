...
# df, df_agro, field_data, field_data_orbit, theta_field, sm_field, height_field, lai_field, vwc_field, pol_field, vv_field, vh_field = read_data(path, file_name, extension, field, path_agro, file_name_agro, extension_agro, pol)
    ...
    for k in surface_list:
        for kk in canopy_list:
            ...
            for kkk in opt_mod:
                ...
                if canopy == 'turbid_isotropic':
                    vv_model, vh_model =solve_fun([np.array(df_output[k, kk, kkk, field, 'coef'])], var_opt, dic, models)

                elif canopy == 'water_cloud':
                    vv_model, vh_model =solve_fun([np.array(df_output[k, kk, kkk, field, 'B_vv']), np.array(df_output[k, kk, kkk, field, 'B_vh'])], var_opt, dic, models)
    ...
    for k in surface_list:
        for kk in canopy_list:
            for kkk in opt_mod:
                ...
                plt.legend()
                plt.savefig(plot_output_path+'validation/'+field+k+kk+'.png')
                plt.close()
...
for k in surface_list:
    for kk in canopy_list:
        for kkk in opt_mod:
            ...
            # s1_vv, model_vv = nan_values(s1_vv,model_vv)
...
# xxx = '95'
xxx = ''
df_insitu = pd.read_csv(csv_output_path+'all_50'+xxx+'.csv',header=[0,1,2,3,4,5],index_col=0)
    ...
    for kk in canopy_list:
        for k in surface_list:
            ...
            for field in field_list:
                ...
                ax.plot(10*np.log10(vv_model.values.flatten()), 10*np.log10(vv_s1.values.flatten()), colors[jj] , linestyle='', marker='.', label=field + ': RMSE: '+ str(rmse)[0:4] + '; R2: ' + str(r_value)[0:4])
...
# ### Nur time_invariant ersten paar Tage zur Berechnung von C_vv und D_vv (überflüssig)
...
#             df, df_agro, field_data, field_data_orbit, theta_field, sm_field, height_field, lai_field, vwc_field, pol_field, vv_field, vh_field, vwcpro_field = read_data(path, file_name, extension, field, path_agro, file_name_agro, extension_agro, pol)
#             freq = 5.405
#             clay = 0.08
#             sand = 0.12
#             bulk = 1.5
...
#                 if canopy == 'turbid_isotropic' and surface == 'WaterCloud':
#                     vv_model, vh_model =solve_fun([np.array(df_output[k, kk, kkk, field, 'coef']), np.array(df_output[k, kk, kkk, field, 'C_vv']), np.array(df_output[k, kk, kkk, field, 'D_vv']), np.array(df_output[k, kk, kkk, field, 'C_vh']), np.array(df_output[k, kk, kkk, field, 'D_vh'])], var_opt, dic, models)
#                 elif canopy == 'turbid_isotropic':
#                     vv_model, vh_model =solve_fun([np.array(df_output[k, kk, kkk, field, 'coef'])], var_opt, dic, models)
#                 # elif surface == 'WaterCloud' and canopy == 'water_cloud' and kkk == 'time_invariant':
#                 #     vv_model, vh_model =solve_fun([np.array(df_output[k, kk, kkk, field, 'A_vv']), np.array(df_output[k, kk, kkk, field, 'B_vv']), np.array(df_output[k, kk, kkk, field, 'A_vh']), np.array(df_output[k, kk, kkk, field, 'B_vh']),aaa[4],aaa[5],aaa[6],aaa[7]], var_opt, dic, models)
#                 elif surface == 'WaterCloud' and canopy == 'water_cloud':
#                     vv_model, vh_model =solve_fun([np.array(df_output[k, kk, kkk, field, 'A_vv']), np.array(df_output[k, kk, kkk, field, 'B_vv']), np.array(df_output[k, kk, kkk, field, 'A_vh']), np.array(df_output[k, kk, kkk, field, 'B_vh']), np.array(df_output[k, kk, kkk, field, 'C_vv']), np.array(df_output[k, kk, kkk, field, 'D_vv']), np.array(df_output[k, kk, kkk, field, 'C_vh']), np.array(df_output[k, kk, kkk, field, 'D_vh'])], var_opt, dic, models)
#                 elif canopy == 'water_cloud' and surface != 'WaterCloud':
#                     vv_model, vh_model =solve_fun([np.array(df_output[k, kk, kkk, field, 'A_vv']), np.array(df_output[k, kk, kkk, field, 'B_vv']), np.array(df_output[k, kk, kkk, field, 'A_vh']), np.array(df_output[k, kk, kkk, field, 'B_vh'])], var_opt, dic, models)
