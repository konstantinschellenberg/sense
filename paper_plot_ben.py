...
bias_output = pd.DataFrame()
ubrmse_output = pd.DataFrame()
    ...
    for k in surface_list:
        for kk in canopy_list:
            for kkk in opt_mod:
                ...
                vv_modeled.to_csv('/media/tweiss/Work/z_check_data/new/'+'modeled_esu.csv')

                rmse = rmse_prediction(10*np.log10(vv_validation[field].values),10*np.log10(vv))
                rmse_output[field,k,kk] = np.array([rmse])
                rmse_output.to_csv('/media/tweiss/Work/z_check_data/new/'+'rmse_esu.csv')

                bias = bias_prediction(10*np.log10(vv_validation[field].values),10*np.log10(vv))

                ubrmse = ubrmse_prediction(rmse,bias)
                ubrmse_output[field,k,kk] = np.array([ubrmse])
                rmse_output.to_csv('/media/tweiss/Work/z_check_data/new/'+'ubrmse_esu.csv')

                plt.plot(10*np.log10(vv),color='blue', label='RMSE:'+str(rmse)[0:4]+' ubrmse:'+str(ubrmse)[0:4]+' bias:'+str(bias)[0:4])
                plt.plot(10*np.log10(vv_validation[field].values), color='black')
                plt.plot(10*np.log10(vv)+bias,color='green')
                plt.legend()
